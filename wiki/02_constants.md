...
CdS dissipation: Fröhlich polar-optical e-ph is the **primary** room-T channel. The impact-ionization **prefactor** is a fit parameter (no cited CdS value) — enabling II without an explicit `sbe_ii_prefactor` aborts. **Carrier-carrier (e-e) is FORBIDDEN for CdS** (`eeh_ok=.false.`): there is no cited CdS carrier-carrier *rate*, so the implemented FD-thermalization channel would have to borrow the generic 1×10¹⁴ s⁻¹ scale cited only for GaAs/Si — forbidden by the provenance rule. The CdS literature gives an e-e *timescale* (sub-100 fs at n ≳ 1×10¹⁸ cm⁻³ [Shah 1986; Elsaesser 1991]), not a rate. **There is no verified CdS Auger coefficient.** The value formerly carried as "C = 2.0×10⁻³⁰ cm⁶/s [Haury, PRB 57, 11513 (1998)]" was a **fabricated reference** — the real Haury et al. paper is PRL 79, 511 (1997) on ferromagnetism in CdMnTe quantum wells, unrelated to Auger in CdS, and the coefficient is unconfirmed anywhere. It has been **removed**; `CDS_AUGER_C` no longer exists and CdS Auger is gated off (`auger_ok=.false.`). `CDS_EE_ACT_N` (Shah 1986) is retained only as a density gate for a user who supplies their own coefficient. A user with their own CdS e-e rate can still opt in by setting `sbe_eeh_nu_sat` explicitly (same explicit-input escape hatch as the II prefactor); likewise a verified Auger C via `sbe_auger_c_cm6s`. The **piezoelectric acoustic** (e₃₃/e₃₁/e₁₅ [Berlincourt 1963]) and **deformation-potential acoustic** channels are cited but not yet SBE Lindblad channels (future). ⛔ = enabling it for that material aborts; nothing is borrowed between materials. 🚧 = the constants are cited and documented but the SBE channel/material wiring is in progress.
...
| `coulomb_ok / eph_ok / ii_ok` | `.true.` | provenance gates | Coulomb/e-ph/II cited |
| `eeh_ok` | `.false.` | carrier-carrier gate | **no cited CdS e-e rate → forbidden** |
| `auger_ok` | `.false.` | Auger gate | **no verified CdS Auger C → forbidden** (the "Haury 1998" value was fabricated) |
| `auger_c_cm6s / auger_n_gate_cm3` | 0 / 1.0e18 | Auger recombination (`yn_sbe_auger`) | no verified C (removed); n_gate Shah 1986 (used only with a user C) |

**There is no verified CdS Auger coefficient.** The value formerly carried as `CDS_AUGER_C` = 2.0e-30 cm⁶/s [Haury et al., PRB 57, 11513 (1998)] was a **fabricated reference** (the real Haury et al. paper is PRL 79, 511 (1997) on CdMnTe ferromagnetism, unrelated) and has been **removed**. `CDS_EE_ACT_N` = 1.0e18 cm⁻³ [Shah et al., IEEE JQE 22, 1728 (1986)] is retained only as a density gate for a user who supplies their own coefficient via `sbe_auger_c_cm6s`.

Every remaining CdS constant carries its source. The impact-ionization **prefactor** is an uncited fit parameter, so it stays a sentinel and the run aborts unless the user supplies `sbe_ii_prefactor`. **Carrier-carrier (e-e) and Auger are both forbidden** (`eeh_ok=.false.`, `auger_ok=.false.`): no cited CdS rate / no verified Auger C. Piezoelectric (e₃₃/e₃₁/e₁₅ [Berlincourt 1963]) and deformation-acoustic are cited but not yet SBE channels.
...
| ~~CdS Auger coeff (n-type)~~ | ~~2.0e-30 cm⁶/s~~ — **REMOVED: fabricated citation** (real Haury et al. = PRL 79, 511 (1997), CdMnTe ferromagnetism, unrelated; coefficient unconfirmed) | — |
...
## 13. Lindblad jump operators (Auger recombination) — ✅ channel wired (k-local); ⛔ no cited C yet

**Implemented** (`apply_auger_recombination`, bloch_solver_ssbe.f90): a density-gated, number-conserving CPTP channel on the gap-edge Houston branches. A conduction electron (ic1) recombines with a valence hole (iv1) — destroying an e–h pair — and the released gap energy promotes a second ic1 electron to the energy-matched hot conduction state ic_hot. Both transfers are `amp_damp_channel` GKLS maps at the per-carrier rate γ = C·n² (so R = C·n³), with occ_max-normalized [0,1]-clamped Pauli factors (CB electron present, VB hole present, hot target empty). Inert below `n_gate`. **Auger acts on the real (Houston/adiabatic) populations**, so with a typically tiny C it is a *rare* event — visible only at very high real-carrier density / strong fields; its role is to be present and exactly CPTP. **Provenance:** no material currently ships a verified C — the former CdS default (C=2.0e-30 [Haury 1998]) was a fabricated citation and was removed, so CdS Auger is now **forbidden** (`auger_ok=.false.`) along with GaAs/Si/graphene; a plain `yn_sbe_auger='y'` run `error stop`s unless the user supplies `sbe_auger_c_cm6s`. CPTP invariants in `tests/test_auger_cptp.f90` (number conserved, recombining, PSD, Hermitian, γ=0 identity, Pauli in [0,occ]).

> **This k-local channel is superseded by the nonlocal-Auger task (`wiki/07_nonlocal_auger.md`):** the current version fixes the initial states at the band extrema (Γ / valley minima) — the first-generation impact-ionization approximation. The new task rebuilds Auger as a fully **nonlocal** detailed-balance partner of the (already nonlocal) impact-ionization kernel, with cited per-material coefficients for **GaAs [Steiauf 2014], Si [Laks 1990], graphene [Rana 2007]**, enabled only when the ring (`yn_sbe_superres`) is on. **CdS is disabled** (no verified coefficient).
...
**Rate / gating.** The Auger rate per unit volume is `R_Auger = C·n³` (electron-density form) with a per-material coefficient **C** (no verified value currently shipped — see the nonlocal-Auger task for the cited GaAs/Si/graphene coefficients); the channel is **density-gated** — it is inert below `ee_activation_n` (e.g. the retained CdS gate 1×10¹⁸ cm⁻³ [Shah 1986]) and turns on as the photoexcited density crosses it. For **graphene** the process is **nearly thresholdless** (gaplessness → carrier multiplication, CM factor up to ~2 [Plötzing 2014]); the collinear-scattering phase space is regularized by the lifetime broadening generated self-consistently by the e-ph + e-e rates (not an ad-hoc width). Screening choice (`static`|`dynamical` RPA) is a modeling switch — static maximizes CM/AR, dynamical can suppress it [Tomadin et al., PRB 88, 035430 (2013)].

**CPTP invariants (tested where implemented):** trace (carrier number) conserved; populations stay in [0, occ]; Hermiticity preserved; γ=0 → identity. Sources: Taj-Rossi PRA 78, 052113 (2008); Rosati et al. PRB 90, 125140 (2014). (Cited per-material Auger coefficients: see `wiki/07_nonlocal_auger.md` — Steiauf 2014 / Laks 1990 / Rana 2007.)
