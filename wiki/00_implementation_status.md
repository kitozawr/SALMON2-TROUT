...
- **CdS (wurtzite P6₃mc):** added as STRUCTURE ONLY. Cell = orthorhombic `al(1:3)=(a,a√3,c)` (not a single cubic constant; the SBE side already takes an al vector). EPM form factors are the REAL cited values from **Bergstresser & Cohen, Phys. Rev. 164, 1069 (1967), Table II** (local potential — the paper assumes spherical atomic potentials, no nonlocal term, so `rvnl_tm=0`; no cited CdS nonlocal parameter exists, so none is fabricated). **All CdS dissipation channels are forbidden** (no cited rate constants). The local band SOLVE does NOT yet reproduce the paper's 2.58 eV gap (~13 eV — convention/basis WIP) and is NOT shipped as physical (`validate_against_paper()` returns False; the test checks only geometry + cited form factors). `epm_wurtzite_cds.py`, `tests/test_wurtzite_cds_epm.py`.
