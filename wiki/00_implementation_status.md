...
2. **Fortran EPM for non-cubic primitive cells** (TODO-2) — 🟢 **MOSTLY DONE (2026-06-30):** the in-SALMON `src/epm` now builds the **non-orthogonal PRIMITIVE cell** as the DEFAULT (`epm_cell='primitive'`) and is **verified interchangeable** with the Python primitive refs — **GaAs** (FCC 2-atom, 5e-11 Ha), **Si** (5e-11 Ha), **graphene** (2-atom hexagonal, ~6e-8 eV). Maintainer decision: *primitive is the default for ALL materials*; `epm_cell='folded'` is the legacy cubic-supercell opt-in (GaAs/Si only, feeds the SBE unfold pipeline). **Still pending:** the **CdS wurtzite** primitive path (4-atom polar, BC1967 form factors — currently `error stop`, use `epm_cds_primitive.py`) and the **spinor** Fortran path. The whole EPM→SBE primitive pipeline now runs in-SALMON for GaAs/Si/graphene (`samples/exercise_x7/*_epm_gs.inp`).
