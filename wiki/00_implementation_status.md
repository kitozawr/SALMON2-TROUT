...
**OPEN tasks (genuinely remaining):**
0. **🔭 NONLOCAL AUGER (NEW, maintainer 2026-06-30) — the next major physics task. Full spec: [`wiki/07_nonlocal_auger.md`](07_nonlocal_auger.md).** Rebuild Auger from the current **k-local** (Γ-pinned) channel into a fully **nonlocal** one by reusing the **already-nonlocal impact-ionization kernel** (they are time-reverses → same |M|², swap the Fermi factors; §5 of wiki/07). Combine the existing **e-ph matrix element + screened-Coulomb/HF + nonlocal-II** machinery. Cited coefficients: **GaAs [Steiauf 2014], Si [Laks 1990], graphene 2D branch [Rana 2007]**; **CdS disabled** (no verified C). Must keep the umklapp **G-sum** and **q-dependent ε(q)** (dropping either underestimates ~10×). **Ring-gated** (`yn_sbe_superres='y'`). ⚠️ **LARGE SBE edit → own feature branch; validate by calculation (CPTP + population maps), NOT unit tests alone; send the maintainer plots.** ✅ *done this session:* CdS Auger disabled + the fabricated Haury coefficient purged (see decisions log).
1. **graphene dissipators (e-ph)** — add the `epm_material='graphene'` registry entry (E2g 196 meV, A1′ 160 meV; no-Kuhn-Zurek assertion). NOTE **gapless physics**: the gap-based II threshold doesn't apply as-is. (The graphene Auger/gapless-CM part is now folded into the nonlocal-Auger task above, item 0, via the 2D Rana branch.)
