...
- **`nex` cell-volume.** The excited-carrier density uses `n = (BZ-avg excited e)/gs%volume` (`update_excited_density`, `nex_density`, current `jmat /(Σkweight·gs%volume)`). Now that we run **non-orthogonal primitive cells**, verify `gs%volume` is the true primitive cell volume $a_1\!\cdot\!(a_2\times a_3)$ (from the `al_vec`/k.data header), not a cubic $a^3$ — otherwise the density/current absolute scale is wrong (band *occupations*/maps are unaffected). Check per material (FCC primitive V=a³/4, wurtzite hexagonal, graphene-in-vacuum).
