...
1. **inter-k e-ph through the ring — ✅ DONE + verified** (branch `claude/inter-k-ring-eph-ii`, commit 71e8291): `eph_interk_dpop` (CPTP core, unit-tested) + `apply_eph_interk_ring` (gather Houston spectrum + per-k apply), gated `flag_eph.and.flag_ring`. Primitive Si 8³ ran 2000 steps, trace=8.00000 conserved. **Next on this branch: momentum-conserving nonlocal impact ionization** (maintainer: "the ring should also run the k-nonlocal impact ionization — momentum conservation + Fermi golden rule"). DESIGN: the current `apply_impact_ionization` nonlocal mode (`flag_nl_ii`) uses BZ-AVERAGED partner/Pauli (`glob_occ`) and creates the pair at the SAME k — NOT momentum-resolved. The ring version must do the true 2-particle event: hot e⁻ (k1,c)+valence e⁻ (k2,v) → (k1',c)+(k2',c) with **crystal-momentum conservation k1+k2 = k1'+k2' (mod G)** and an **energy-conserving Fermi golden rule** rate `∝ |V_scr(k1−k1')|² δ(E_c(k1)+E_v(k2)−E_c(k1')−E_c(k2'))·Pauli`. Needs: (a) a k-index map `idx(k1+k2−k1')` for the regular MP grid (reduced-coord arithmetic + wrap) — a reusable utility (also useful for inter-k e-e); (b) a pure CPTP `ii_interk_dpop(eval,f,kmap,V_scr,…)→dpop` mirroring `eph_interk_dpop` (trace-conserving: source losses = final gains; e–h pair number bookkeeping); (c) a `test_ii_interk_cptp.f90`; (d) `apply_ii_interk_ring` gathering eval/f and applying, gated `flag_impact.and.flag_ring`, k-local II gated off when ring on. O(nk²·nba³) (k1,k1' sums × bands; k2 from the carrier pool, k2' fixed by momentum). Ride/extend the existing Coulomb ring for `V_scr`.
...
4. ✅ **DONE.** `--spectral` colours all FOUR gap-edge bands (2 valence + 2 conduction). The primitive SBE writes `SYSNAME_sbe_nex_k_lev_real.data` (diabatic VB-1/VB/CB1/CB2 per k, t=0 = real equilibrium); `plot_primitive_spectral` colours holes (occ−pop) in valence and electrons (pop) in conduction on a shared carrier scale (LCB-only fallback when absent). Verified on GaAs primitive.
