...
**OPEN tasks — ORDER IS FILO (maintainer 2026-06-30: "auger i last"). Work top-down; the most-recently-added task (nonlocal Auger) is done LAST.**
...
3. **deeppseudodot DFT→EPM** (TODO-7) — branch `claude/dft-epm-coefficients-pdvguw`; DFT(primitive Si, in-salmon) → fit EPM coefficients; drop a ready Si example in `samples/`.
4. **Examples + docs** (#14, ongoing) — keep `samples/` + wiki current as features land (readme+wiki = long-term memory).
5. **🔭 NONLOCAL AUGER — DO LAST (FILO, maintainer 2026-06-30: "auger i last"). Full spec: [`wiki/07_nonlocal_auger.md`](07_nonlocal_auger.md).** Rebuild Auger from the current **k-local** (Γ-pinned) channel into a fully **nonlocal** one by reusing the **already-nonlocal impact-ionization kernel** (they are time-reverses → same |M|², swap the Fermi factors; §5 of wiki/07). Combine the existing **e-ph matrix element + screened-Coulomb/HF + nonlocal-II** machinery. Cited coefficients: **GaAs [Steiauf 2014], Si [Laks 1990], graphene 2D branch [Rana 2007]**; **CdS disabled** (no verified C). Must keep the umklapp **G-sum** and **q-dependent ε(q)** (dropping either underestimates ~10×). **Ring-gated** (`yn_sbe_superres='y'`). ⚠️ **LARGE SBE edit → own feature branch; validate by calculation (CPTP + population maps), NOT unit tests alone; send the maintainer plots.** ✅ *prep done 2026-06-30:* CdS Auger disabled + the fabricated Haury coefficient purged (see decisions log); **computation-reuse + consistency rules written up (wiki/07 §0.1/§0.2): share the BARE `coulomb_kernel` + ring + MP-map + GKLS, but ÷ε screening applies to the COLLISION channels only (NOT Σ^HF — no double-count); BGR and Σ^HF must not both renormalise the gap; dynamic screening (LOPC GaAs) is material-specific and the static screen is wrong at the ~1 eV Auger scale (Si λ=0).**
...
- ⬜ **3D Brillouin-zone population plot (maintainer 2026-07-01 — paper-style, NEW TODO).** A true 3D view of the real-carrier population in the Cartesian BZ (like the truncated-octahedron figures in the Auger/impact-ionization papers): draw the **BZ wireframe** (the Wigner-Seitz cell edges — reuse `_bz_outline_2d`'s half-space geometry in 3D) and render the population either as **(a)** Monkhorst-Pack grid points coloured + sized by occupation (`matplotlib` 3D scatter, `s∝pop`, `c=pop`, low-pop points small/faint), or **(b)** a **semi-transparent voxel cloud** with smoothing (weakly-populated voxels transparent — `alpha∝pop`, Gaussian-smoothed `cpop3d`). Feeds off the existing `_cartesian_bz_grid` output (`cpop3d`, `cx/cy/cz`) + the reciprocal `b_matrix`; add as a `--bz3d` snapshot mode. Reference: the Γ-blob (a) vs the 8 L-valley/Δ clusters (b) truncated-octahedron figure the maintainer attached.
