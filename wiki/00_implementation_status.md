...
- ⬜ **Fortran EPM should emit `_bandpath.data`** (only the Python refs do today), so `theory='epm'` → `--spectral` is seamless without a Python step. Add a `bandpath` emission to `src/epm` (the high-symmetry path per material). Needed for the fully in-SALMON primitive EPM→SBE→plot example the maintainer wants.
