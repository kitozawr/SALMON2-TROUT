...
> **Pipeline status (honest):** **all four materials emit SBE ground-state files** (`SYSNAME_k/_eigen/_tm.data`), a clean primitive **band path** (`_bandpath.data`), and an **unfold map** (`_unfold.data`), and run in the SBE. GaAs/Si use the **Fortran `theory='epm'`** path (and the Python ref, 4-coset FCC fold); **CdS and graphene use the Python references** (`python3 epm_wurtzite_cds.py` / `epm_graphene.py`) on their folded supercells (CdS orthorhombic 2-fold; graphene rectangular 4-atom 2-fold — both verified exact, both ran end-to-end in the SBE with trace/electron number conserved). The unfold infrastructure (`gs_info_ssbe`, `bloch_solver`, `datafile`, the plotter) is generalized from the hardcoded **4 FCC cosets to N** (4 = cubic, 2 = wurtzite/rectangular); legacy 4-coset GaAs maps still read. **The Fortran `src/epm` solver now also emits the non-orthogonal PRIMITIVE cell (`epm_cell='primitive'`, default) for GaAs/Si/graphene/CdS** — all verified interchangeable with the Python primitive references — so the whole EPM→SBE primitive pipeline runs in-SALMON for every material (see `samples/exercise_x7_primitive_cell_epm/*_epm_gs.inp`). Still pending in Fortran: the **spinor** primitive path (use the Python `INCLUDE_SPIN_ORBIT` reference). See `wiki/00_implementation_status.md` for the full TODO.
