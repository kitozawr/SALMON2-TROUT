...
| CdS | `epm_wurtzite_cds.py` | wurtzite (polar), orthorhombic `al(1:3)`, **2-fold fold (verified exact)** | direct gap **2.55 eV** vs Bergstresser-Cohen PR 164, 1069 (1967) 2.58 eV | ✅ Python GS-file emission (folded orthorhombic, scalar); ran end-to-end in the SBE (trace conserved). Unfold map pending |
| graphene | `epm_graphene.py` | honeycomb (D6h, V_A=0), **rectangular 4-atom cell + 2-fold fold (verified exact)** | **zero gap** at Dirac K, **v_F=9.6×10⁵ m/s**, Γ=−7.78 eV, M=−2.70 eV [Ramanujam thesis, ASU 2015] | ✅ Python GS-file emission (folded rectangular, π-model, scalar); ran end-to-end in the SBE (trace conserved). Unfold map pending |

> **Pipeline status (honest):** **all four materials now emit SBE ground-state files** (`SYSNAME_k/_eigen/_tm.data`) and run in the SBE. GaAs/Si use the **Fortran `theory='epm'`** path (and the Python ref); **CdS and graphene use the Python references** (`python3 epm_wurtzite_cds.py` / `epm_graphene.py`) on their folded supercells (CdS orthorhombic 2-fold; graphene rectangular 4-atom 2-fold — both verified exact, and both ran end-to-end in the SBE binary with the trace/electron number conserved). **The Fortran `src/epm` solver is cubic-only and is NOT ready for CdS/graphene** — use the Python references for those. Still pending: the band-**unfolding** map (`_unfold.data`) for the 2-fold materials (needs the SBE reader generalized from 4 to N cosets), the **spinor** Fortran path, and the Auger Lindblad channel. See `wiki/00_implementation_status.md` for the full TODO.
