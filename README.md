...
| `yn_sbe_auger` | — | `'n'` | `'y'`: **Auger recombination** (density-gated, number-conserving CPTP; Sec 13). Per-carrier rate $\gamma=C n^2$ (so $R=C n^3$), inert below $n_{\rm gate}$. Acts on real (Houston) carriers ⇒ a *rare* event. Provenance-gated: **no material currently ships a verified $C$**, so it requires an explicit `sbe_auger_c_cm6s` (the cited GaAs/Si/graphene coefficients are the [nonlocal-Auger task](wiki/07_nonlocal_auger.md)). |
...
| `epm_cell` | — | `'primitive'` | `'primitive'`: non-orthogonal primitive cell, NO folding (GaAs/Si FCC 2-atom; graphene hexagonal) — verified interchangeable with the Python primitive refs. `'folded'`: legacy cubic 8-atom supercell + FCC-in-cubic folding (GaAs/Si/Si_cb only; needed for the SBE unfold pipeline). For graphene only `'primitive'` is valid; for CdS the Fortran path is pending (use the Python ref). |
...
* **`epm_cell='primitive'` (default) — non-orthogonal primitive cell, NO folding.** GaAs/Si/Si_cb use the FCC 2-atom cell (the Cohen-Bergstresser Hamiltonian on the single-parity plane-wave basis); graphene uses the 2-atom hexagonal honeycomb (Ramanujam π-EPM, general structure factor). **Verified interchangeable with the Python primitive references** (`epm_gaas_primitive.py` / `epm_si_primitive.py` / `epm_graphene_primitive.py`): identical k-points, band energies to **5×10⁻¹¹ Ha** (GaAs/Si) / **~6×10⁻⁸ eV** (graphene), identical occupations. CdS (wurtzite) is still Python-only (the Fortran wurtzite path is pending).
...
> **Pipeline status (honest):** **all four materials emit SBE ground-state files** (`SYSNAME_k/_eigen/_tm.data`), a clean primitive **band path** (`_bandpath.data`), and an **unfold map** (`_unfold.data`), and run in the SBE. GaAs/Si use the **Fortran `theory='epm'`** path (and the Python ref, 4-coset FCC fold); **CdS and graphene use the Python references** (`python3 epm_wurtzite_cds.py` / `epm_graphene.py`) on their folded supercells (CdS orthorhombic 2-fold; graphene rectangular 4-atom 2-fold — both verified exact, both ran end-to-end in the SBE with trace/electron number conserved). The unfold infrastructure (`gs_info_ssbe`, `bloch_solver`, `datafile`, the plotter) is generalized from the hardcoded **4 FCC cosets to N** (4 = cubic, 2 = wurtzite/rectangular); legacy 4-coset GaAs maps still read. **The Fortran `src/epm` solver now also emits the non-orthogonal PRIMITIVE cell (`epm_cell='primitive'`, default) for GaAs/Si/graphene** — verified interchangeable with the Python primitive references — so the whole EPM→SBE primitive pipeline runs in-SALMON for those (see `samples/exercise_x7_primitive_cell_epm/*_epm_gs.inp`). Still pending in Fortran: the **wurtzite (CdS)** primitive path, the **spinor** path, and the Auger Lindblad channel — use the Python references there. See `wiki/00_implementation_status.md` for the full TODO.
