...
`num_sbe` is not required to enable dephasing; it is an existing SBE setting
that defaults to `1`. Dephasing is controlled only by `t2_sbe_fs`: set it to a
positive value below `1.0d9` to enable T2 damping.
