...
| **Impact ionization** | `yn_sbe_impact_ionization = 'y'` | `'n'` (default) | *auto:* `stobbe_quartic`, $a=4$, $E_{\rm th}=2.1$ | *auto:* `keldysh_quadratic`, $a=2$ ($4.6$ full-band), $E_{\rm th}=1.1$ | ⛔ **forbidden** (no cited $P$) |
| **Electron–phonon** | `yn_sbe_eph = 'y'` (+ `&epm epm_material`) | `'n'` (default) | *auto:* Fröhlich-LO 36 meV + 5 intervalley, `ν_sat=1.0e14` | *auto:* 6 intervalley g/f, `ν_sat=1.3e14` | ⛔ **forbidden** (no cited `ν_sat`) |
...
| **Coulomb HF** | `yn_sbe_coulomb = 'y'` | `'n'` (default) **or** `sbe_coulomb_strength=0` | *auto:* $\varepsilon=12.9$ | *auto:* $\varepsilon=11.7$ | ⛔ **forbidden** unless explicit `sbe_coulomb_epsilon` |
...
**Provenance rule (strict):** a channel is enabled for a material **only** if its constants are backed by a cited source for that material — *no source ⇒ forbidden*, and the run **aborts** (`error stop`) rather than borrow another material's numbers. The per-material constants come from a single **material registry** (`get_material_params` in `src/ssbe/sbe_superres_ssbe.f90`), which carries per-channel provenance gates `ii_ok / eph_ok / eeh_ok / coulomb_ok`. **CdS** (wurtzite) currently supplies only its *structure* (cited form factors, [BC1967](#references--theoretical-background)); all its dissipation channels are ⛔ forbidden because no CdS-specific rate constants are cited — enabling one aborts with a `FORBIDDEN` message. An explicit namelist value (e.g. a cited `sbe_coulomb_epsilon`) overrides the gate. **Adding a material is one cited `case` block.** Runnable per-material recipes are in [`wiki/04_configuration_examples.md`](wiki/04_configuration_examples.md); the full effect-support matrix with sources is in [`wiki/02_constants.md`](wiki/02_constants.md).
