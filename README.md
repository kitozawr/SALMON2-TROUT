...
- **Momentum-conserving nonlocal ring channels** (inter-k e-ph, impact
  ionization and its exact time-reverse Auger through one systolic-ring gather)
  with **CDRB model ε(q) screening + umklapp G-sum** in the Cartesian cell
  metric; the graphene gapless case has its own **2D Rana Auger/CM** branch
  (see [`wiki/07_nonlocal_auger.md`](wiki/07_nonlocal_auger.md))
...
- Empirical Pseudopotential Method (EPM), **fully in-SALMON** for every
  material and spin mode: `theory='epm'` emits GS files, `_bandpath.data`, and
  (spinor GaAs) the nonlocal SO velocity correction — no Python step required
...
- **Real DFT levels into the SBE** (`theory='dft'` + `yn_out_tm='y'` →
  `dft_band` band path → SBE, with the active/frozen-core window) — an
  alternative to the EPM band structure; see
...
A self-contained local-EPM ground-state solver (`theory='epm'`, `src/epm`) that computes the Cohen-Bergstresser band structure and momentum matrix elements directly in SALMON, and writes `SYSNAME_k.data`/`SYSNAME_eigen.data`/`SYSNAME_tm.data` in exactly the format read by `gs_info_ssbe` — closing the EPM → SBE pipeline end-to-end without external scripts. It now covers **all four materials** (GaAs, Si/Si_cb, wurtzite CdS, graphene) on the **non-orthogonal primitive cell by default** (`epm_cell='primitive'`; the legacy cubic `'folded'` supercell for GaAs/Si feeds the band-unfold pipeline), each verified interchangeable with the Python reference to ~5×10⁻¹¹ Ha. It also emits `SYSNAME_bandpath.data` (the clean primitive band path, same contract as `theory='dft_band'` and the plotter) so `--spectral` needs no Python step. For a *local* pseudopotential `rvnl_tm` is identically zero (no nonlocal velocity correction); the **spinor GaAs** path (`yn_spinorbit='y'`, §6) is the exception — it writes the analytic spin-orbit velocity correction $\hat v_{SO}=\nabla_k\hat H_{SO}$ into block 2.

### 6. Spinor (spin-orbit split) EPM input + `yn_sbe_spinor`
Both the **in-SALMON Fortran EPM** (`theory='epm'` with `yn_spinorbit='y'`, plus the input-checker's `spin='noncollinear'`) and the Python reference (`epm_gaas_reference.py`, hardcoded flag `INCLUDE_SPIN_ORBIT = True`) promote the scalar $N_{PW}$ problem to the **spinor $2N_{PW}$ problem** — the two are verified interchangeable (eigenvalues and band path to 5×10⁻¹¹ Ha, gauge-invariant Kramers-doublet momenta to ~1e-9; cited GaAs-only $\Delta_0$). The plane-wave basis is doubled to $|G,s\rangle$, and
...
### 7. Impact ionization (optional Lindblad channel, `yn_sbe_impact_ionization`)
A fully **optional** carrier-multiplication channel (off by default — it can be slow, and for a first estimate the purely coherent + dephasing code is sufficient). Two representations: the **k-local** one described here (each k-point independent, VG parallelism intact), and — when the nonlocal ring is on (`yn_sbe_superres='y'`) — a **momentum-conserving inter-k** version (`apply_ii_interk_ring`) in which the true two-particle event exchanges crystal momentum $k_1+k_2=k_1'+k_2'\,(\text{mod}\,G)$ through the MP index map, so Si's *indirect* gap is resolved explicitly (the created pair lands in the correct Δ-valleys). The inter-k weight uses the **CDRB model ε(q) + umklapp G-sum** in the Cartesian cell metric (§ nonlocal ring, `wiki/07`). The k-local description below is the baseline; both share the same Stobbe/Keldysh magnitude. Hot conduction electrons with kinetic energy above the threshold ionize a valence electron, creating a "cold" e–h pair at the band edges, with the rate taken from the verified **Stobbe–Redmer–Schattke fit** (PRB 49, 4494) to a full Fermi-golden-rule calculation on the *same* Cohen-Bergstresser band structure as our EPM:
...
**Declared limitations of the fit** (Stobbe): direction-averaged (their matrix elements are nearly isotropic — energy is the dominant variable); electron-initiated channel only (hole-initiated omitted); no phonon-assisted ionization, collisional broadening, or field-induced threshold softening (Quade–Schöll–Rossi: at MV/cm there is strictly no fixed threshold — near-threshold rates are underestimated, a known limitation); fit energy resolution $\delta E = 0.2$ eV (the $\Theta$ step is smoothed by a linear ramp of this width). *(These limitations are of the k-local Stobbe fit; the nonlocal-ring impact ionization and its detailed-balance Auger partner — `yn_sbe_auger` with `yn_sbe_superres` — lift the Γ-pinning of the initial states, see §8-below and `wiki/07`. Electron–electron scattering is the separate `yn_sbe_eeh` channel.)*
...
* from `SYSNAME_sbe_rt.data`, the **optical conductivity** $\sigma(\omega) = J(\omega)/E(\omega)$ along the driven axis: a global spectrum with $\mathrm{Re}\,\sigma$ and $\mathrm{Im}\,\sigma$ on two $Y$-axes over $0$–$4$ THz, and a strongly-overlapped **short-time-Fourier** $\mathrm{Re}\,\sigma(\omega,t)$ 2-D map (Hann-windowed; the $J/E$ ratio is regularised and pixels with no local drive power are blanked). Tune with `--fmax-thz`, `--stft-window-fs`, `--stft-hop`, or disable with `--no-conductivity`. Resolving the $0$–$4$ THz band needs a ps-scale run (the script warns and shows the lowest bins otherwise);
...
#### Analytic field-coupling & Zener-tunnelling probes (GaAs)
Two standalone analysis scripts reuse the same `epm_gaas_reference.py` band machinery (same parity-folding, spin-orbit and momentum routines) to ask *why* field-driven carriers appear off the field axis in the unfolded maps:
- **`band_field_coupling.py`** — the unfolded bands, group velocity, **field-projected interband coupling** $\lvert\langle cb\lvert p_{\hat e}\rvert v\rangle\rvert^2$ over the Γ₈ manifold, and the direct gap, along `[100]/[111]/[110]`. Quantifies that the diagonal occupation is band-**folding**, not a matrix-element effect.
- **`zener_tunneling_estimate.py`** — the transverse-$k$-resolved vertical **Zener/Kane** tunnelling probability $P_{\rm Kane}(k_\perp)=\exp(-C\,m_r^{1/2}E_g^{3/2}/F)$ and the linear **Landau–Zener** probability at genuine off-Γ small-gap avoided crossings, vs field. Shows the vertical channel collapsing onto the axis. The `--map2d` mode adds the **2-D transverse birth map** $W(k_y,k_z)$ at $k_\parallel{=}0$: it reads the FWHM of the injection "needle" at Γ directly and overlays the folded conduction-band geometry, so the diagonal weight in the SBE `pop_lcb` snapshots can be seen to coincide with the **fold** positions (zone-edge / L-valley diagonals via the $k_z$-average), not with the injection blob — and its appearance already at the first nonzero step is the signature of folding, not transport.
- **`si_three_photon_isosurfaces.py`** — the **Si MULTIPHOTON** counterpart to the Zener script: a power-law $I^N$ process with a different prefactor, not a tunnelling exponential. Computes the N-photon direct-transition rate $W_N(k)\sim I^N\,\lvert M_N(k)\rvert^2\,g_{\rm FK}\!\big(E_{\rm dir}(k)-N\hbar\omega\big)$ on a 3-D BZ grid: full LOPT ladder over all intermediate bands, exact polarization orientation-average (keeps $O_h$ ⇒ evaluated in the IBZ wedge), and **Franz–Keldysh field broadening** $\hbar\theta=(F^2/2\mu)^{1/3}$ whose sub-edge Airy tail lets the below-edge 3-photon drive ($3\hbar\omega=3.0$ eV $<$ $E_{\rm dir}^{\min}=3.34$ eV) reach the gap — only along the low-direct-gap **Γ–L ⟨111⟩ valleys** — while 4-photon (4.0 eV) is a genuine resonance shell reaching toward (never at) X. The $(E_0/2\omega)^{2N}$ prefactor puts the orders on ONE absolute scale so $W_{\rm tot}=W_3+W_4$ is summable (4γ share ≈4%→34% from 3→10 MV/cm). Outputs: plotly 3-D rate isosurfaces / $E_{\rm dir}$ resonance shells / rate cloud per order, the summed rate + three axis-averaged projections (Agg PNG + interactive HTML), an MRI-style ortho-slicer (`--slice-quantity`), and Γ-L/Γ-X/Γ-K line scans vs field. The SBE **br3d** population maps show the same Γ–L diagonal pattern (see the x8 README note): HHG carriers are seeded along Γ–L by this vertical multiphoton transition, then drift to the Δ valleys. Per-k FK weight gotcha in the wiki/00 decisions log (do **not** use the Airy JDOS lineshape per-k — it double-counts the k-sum).

> **Spinor velocity (nonlocal SO correction).** For a spinor dataset the physical interband velocity is $\langle m\lvert(k+G)+\nabla_k H_{\rm SO}\rvert n\rangle$ — the local momentum **plus** the nonlocal spin-orbit term (the `rvnl_tm` block the SALMON dataset already carries). `band_field_coupling.primitive_bands_momentum` now adds this $\mu\,\nabla_k H_{\rm SO}$ correction (so the Zener script inherits it); it is a small absolute correction near Γ but a relative O(1) effect for the nearly-$p$-dark heavy-hole transition. **Caveat:** this local CB-only EPM undershoots the gap ($E_g(\Gamma)\approx1.27$ eV vs 1.42/1.52 eV experiment), so quote **Γ-normalized** tunnelling probabilities (panel c) unless you scissor-correct.
...
The wiki also covers the **Silicon EPM** ground state (`epm_material='Si'`, diamond V^A=0) and the optional nonlocal **"super-compute" mode** (`yn_sbe_superres`): ring-pipeline-MPI Hartree-Fock, electron-phonon and carrier-carrier CPTP Lindblad channels, the **momentum-conserving inter-k impact ionization ↔ Auger detailed-balance pair** (with CDRB ε(q) + umklapp G-sum screening; `wiki/07`), density-dependent (band-gap-renormalized) impact-ionization thresholds, and the Maxwell-SBE multiscale adaptation — all gated OFF by default so existing GaAs runs are byte-for-byte unchanged.
...
- **[Nonlocal Auger recombination](wiki/07_nonlocal_auger.md)** — the momentum-conserving inter-k impact ionization ↔ Auger detailed-balance pair, the CDRB model ε(q) + umklapp G-sum weight, the graphene 2D Rana branch, and the per-material coefficient validation (S14/L90/R07, all source-verified).
...
5. **Cohen-Bergstresser Local Pseudopotentials:** Cohen, M. L., & Bergstresser, T. K. "Band Structures and Pseudopotential Form Factors for Fourteen Semiconductors of the Diamond and Zinc-blende Structures." *Phys. Rev.* 141, 789 (1966). **Wurtzite CdS form factors:** Bergstresser, T. K., & Cohen, M. L. "Electronic Structure and Optical Properties of Hexagonal CdSe, CdS, and ZnS." *Phys. Rev.* 164, 1069 (1967) (local EPM; CdS form factors in Table II, gap in Table I).
...
9. **Nonlocal Auger recombination & impact ionization** (the `yn_sbe_auger`/ring channels, `wiki/07`): Laks, D. B., Neumark, G. F., & Pantelides, S. T. "Accurate interband-Auger-recombination rates in silicon." *Phys. Rev. B* 42, 5176 (1990) [L90 — nonlocal umklapp G-sum + q-dependent ε(q); Si Cₙ/Cₚ]; Kioupakis, E., Steiauf, D., Rinke, P., Delaney, K. T., & Van de Walle, C. G. "First-principles calculations of indirect Auger recombination in nitride semiconductors." *Phys. Rev. B* 92, 035207 (2015) [K15 — direct+phonon-assisted formalism, the CDRB model ε(q), α=1.563]; Steiauf, D., Kioupakis, E., & Van de Walle, C. G. "Auger Recombination in GaAs from First Principles." *ACS Photonics* 1, 643 (2014) [S14 — GaAs eeh/hhe coefficients]; Rana, F. "Electron-hole generation and recombination rates for Coulomb scattering in graphene." *Phys. Rev. B* 76, 155431 (2007) [R07 — the 2D Dirac Auger/CM branch].
