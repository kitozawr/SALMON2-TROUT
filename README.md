...
- **`zener_tunneling_estimate.py`** — the transverse-$k$-resolved vertical **Zener/Kane** tunnelling probability $P_{\rm Kane}(k_\perp)=\exp(-C\,m_r^{1/2}E_g^{3/2}/F)$ and the linear **Landau–Zener** probability at genuine off-Γ small-gap avoided crossings, vs field. Shows the vertical channel collapsing onto the axis.
