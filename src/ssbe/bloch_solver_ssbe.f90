module bloch_solver_ssbe
    ...
    public :: s_sbe_bloch_solver, init_sbe_bloch_solver, calc_current_bloch, &
              ...
              apply_kuehn_dephasing_kick, update_kuehn_dx, init_adiabatic_dephasing
    ...
    type s_sbe_bloch_solver
        ...
        ! ZHEEV данные
        integer :: lwork_zheev
        logical :: adiabatic_decoh_initialized = .false.
    
        ! Kuehn 2010 non-Markovian dephasing
        real(8) :: int_A(3) = 0.0d0           ! Интеграл от A(t)
        real(8) :: dx_global = 0.0d0           ! Глобальное разделение e-h
        real(8) :: inv_mr_eff = 27.1d0         ! 1/m_c + 1/m_v (для GaAs)
        real(8) :: temperature_K = 80.0d0      ! Температура решетки
        real(8) :: tau_momentum_fs = 1000.0d0  ! Время импульсной релаксации
    end type

contains

subroutine init_adiabatic_dephasing(sbe, gs)
    implicit none
    ...
    complex(8), allocatable :: H_dummy(:,:), evals_dummy(:)
    complex(8) :: work_query(1)
    
    nb = sbe%nb
    if (nb <= 0) then
        write(*,*) "FATAL: nb <= 0 in init_adiabatic_dephasing"
        stop
    end if
    
    
    allocate(H_dummy(nb, nb), evals_dummy(nb))
    allocate(rwork(max(1, 3*nb - 2)))
    
    H_dummy = dcmplx(0d0, 0d0)
    
    ! Workspace query для ZHEEV
    call ZHEEV('V', 'U', nb, H_dummy, nb, evals_dummy, work_query, -1, rwork, info)
    
    if (info == 0) then
        lwork = int(real(work_query(1)))
        sbe%lwork_zheev = lwork
        write(*,'(A, I0, A, I0)') " [ZHEEV INIT] Success: nb=", nb, ", optimal LWORK=", lwork
        write(*,'(A, F8.2, A)') " [KUEHN DECOH] Temperature = ", sbe%temperature_K, " K"
        write(*,'(A, F8.2, A)') " [KUEHN DECOH] tau_momentum = ", sbe%tau_momentum_fs, " fs"
    else
        write(*,*) "FATAL: ZHEEV workspace query failed, info=", info
        stop
    end if
    
    deallocate(H_dummy, evals_dummy, rwork)
    sbe%adiabatic_decoh_initialized = .true.
end subroutine init_adiabatic_dephasing

subroutine update_kuehn_dx(sbe, Ac, dt)
    implicit none
    type(s_sbe_bloch_solver), intent(inout) :: sbe
    real(8), intent(in) :: Ac(3)
    real(8), intent(in) :: dt
    
    ! Интегрируем векторный потенциал
    sbe%int_A(1) = sbe%int_A(1) + Ac(1) * dt
    sbe%int_A(2) = sbe%int_A(2) + Ac(2) * dt
    sbe%int_A(3) = sbe%int_A(3) + Ac(3) * dt
    
    ! Вычисляем глобальное разделение
    sbe%dx_global = sqrt(sbe%int_A(1)**2 + &
                         sbe%int_A(2)**2 + &
                         sbe%int_A(3)**2) * sbe%inv_mr_eff
end subroutine update_kuehn_dx

subroutine apply_kuehn_dephasing_kick(sbe, gs, Ac, dt_kick)
    use phys_constants, only: au_fs
    implicit none
    type(s_sbe_bloch_solver), intent(inout) :: sbe
    type(s_sbe_gs_info), intent(in) :: gs
    real(8), intent(in) :: Ac(1:3)
    real(8), intent(in) :: dt_kick
    
    integer :: ik, n, m, i, j, in, im, idir, nb, nba, info, lwork ! <--- ИСПРАВЛЕНО (добавлены i,j,in,im)
    real(8) :: kb_au, prefactor, gamma_rate, tau_decoh_fs, decay_factor
    real(8), allocatable :: evals(:), rwork(:)
    complex(8), allocatable :: H_inst(:,:), U(:,:), rho_act(:,:), rho_tilde(:,:), tmp1(:,:), tmp2(:,:), rho_act_new(:,:), work(:)

    nb = sbe%nb
    nba = sbe%n_active_bands
    lwork = sbe%lwork_zheev
    
    ! Проверка инициализации
    if (lwork < max(1, 2*nba - 1)) then
        write(*,*) "FATAL: ZHEEV not initialized properly for nba=", nba
        stop
    end if

    ! ---------------------------------------------------------
    ! ВЫЧИСЛЕНИЕ СКОРОСТИ ДЕКОГЕРЕНЦИИ (Формула Зурека-Кюна)
    ! ---------------------------------------------------------
    kb_au = 3.166811563d-6  ! Постоянная Больцмана в а.е. (Ha/K)
    
    ! prefactor = (k_B * T) / tau_m  (в атомных единицах времени)
    prefactor = (kb_au * sbe%temperature_K) / (sbe%tau_momentum_fs / au_fs)
    
    ! gamma_rate = prefactor * (Delta x)^2
    gamma_rate = prefactor * (sbe%dx_global**2)
    
    ! Переводим в tau_decoh (в фс) и ограничиваем снизу (защита от Зенона)
    tau_decoh_fs = 1.0d10
    if (gamma_rate > 1.0d-10) then
        tau_decoh_fs = au_fs / gamma_rate
        tau_decoh_fs = max(tau_decoh_fs, 0.01d0) ! Минимум 0.01 фс, как у Kuehn
    end if
    
    decay_factor = exp(-dt_kick / tau_decoh_fs)

    !$omp parallel default(shared) private(ik, n, m, idir, info, H_inst, U, evals, &
    !$omp                                  rho_act, rho_tilde, tmp1, tmp2, rho_act_new, work, rwork)
    
    allocate(H_inst(nba, nba), U(nba, nba), evals(nba))
    allocate(rho_act(nba, nba), rho_tilde(nba, nba), tmp1(nba, nba), tmp2(nba, nba), rho_act_new(nba, nba))
    allocate(work(lwork), rwork(max(1, 3*nba - 2)))
    
    !$omp do
    do ik = sbe%ik_min, sbe%ik_max
        
        ! 1. Строим H_inst ТОЛЬКО для активных зон (H0 + A*p)
        H_inst(:,:) = dcmplx(0d0, 0d0)
        do m = 1, nba
            im = sbe%active_idx(m)
            do n = 1, nba
                in = sbe%active_idx(n)
                do idir = 1, 3
                    H_inst(n, m) = H_inst(n, m) + Ac(idir) * gs%p_tm_matrix(in, im, idir, ik)
                end do
            end do
            H_inst(m, m) = H_inst(m, m) + gs%eigen(im, ik)
        end do
        
        ! 2. Диагонализация (ZHEEV)
        call ZHEEV('V', 'U', nba, H_inst, nba, evals, work, lwork, rwork, info)
        if (info /= 0) then
            write(*,*) "FATAL: ZHEEV failed at ik=", ik, " info=", info
            stop
        end if
        U(:,:) = H_inst(:,:) 
        
        ! 3. Извлекаем активный блок rho
        do m = 1, nba
            im = sbe%active_idx(m)
            do n = 1, nba
                in = sbe%active_idx(n)
                rho_act(n, m) = sbe%rho(in, im, ik)
            end do
        end do
        
        ! 4. Прямое преобразование: rho_tilde = U^dagger * rho_act * U
        call ZGEMM('N', 'N', nba, nba, nba, dcmplx(1d0, 0d0), rho_act, nba, U, nba, dcmplx(0d0, 0d0), tmp1, nba)
        call ZGEMM('C', 'N', nba, nba, nba, dcmplx(1d0, 0d0), U, nba, tmp1, nba, dcmplx(0d0, 0d0), rho_tilde, nba)
        
        ! 5. Применяем НЕ-МАРКОВСКУЮ декогеренцию (ГЛОБАЛЬНЫЙ decay_factor)
        do m = 1, nba
            do n = 1, nba
                if (n /= m) then
                    rho_tilde(n, m) = rho_tilde(n, m) * decay_factor
                end if
            end do
        end do
        
        ! 6. Обратное преобразование: rho_act_new = U * rho_tilde * U^dagger
        call ZGEMM('N', 'N', nba, nba, nba, dcmplx(1d0, 0d0), U, nba, rho_tilde, nba, dcmplx(0d0, 0d0), tmp2, nba)
        call ZGEMM('N', 'C', nba, nba, nba, dcmplx(1d0, 0d0), tmp2, nba, U, nba, dcmplx(0d0, 0d0), rho_act_new, nba)
        
        ! 7. Записываем обратно в sbe%rho
        do m = 1, nba
            im = sbe%active_idx(m)
            do n = 1, nba
                in = sbe%active_idx(n)
                sbe%rho(in, im, ik) = rho_act_new(n, m)
            end do
        end do
    ...
    deallocate(H_inst, U, evals, rho_act, rho_tilde, tmp1, tmp2, rho_act_new, work, rwork)
    !$omp end parallel
    
end subroutine apply_kuehn_dephasing_kick
